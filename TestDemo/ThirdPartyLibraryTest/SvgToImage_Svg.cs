...
{
    public class SvgToImage_Svg
    {
        const string outputPath = "../../../Output/SvgToImage/";
        ...
        private void Execute(Dictionary<string, string> dic)
        {
            foreach (var pair in dic)
            {
                try
                {
                    ...
                    bitmap.Save($"{outputPath}test_{pair.Key}.jpg", ImageFormat.Jpeg);
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"{nameof(SvgToImage_Svg)} {pair.Key} exception: {ex.Message}");
                }
            }
        }
    }
